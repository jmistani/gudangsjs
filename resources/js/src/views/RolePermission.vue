...
import PermissionLeftSidebar from './PermissionLeftSidebar.vue'
...
export default {
  ...
  setup() {
    ...
    const taskTags = [
      { name: 'kasir', color: 'primary', route: { name: 'apps-todo-tag', params: { tag: 'kasir' } } },
      { name: 'staff-gudang', color: 'success', route: { name: 'apps-todo-tag', params: { tag: 'staff-gudang' } } },
      { name: 'super-admin', color: 'warning', route: { name: 'apps-todo-tag', params: { tag: 'super-admin' } } },
    ]

    const resolveTagVariant = tag => {
      if (tag === 'kasir') return 'primary'
      if (tag === 'staff-gudang') return 'success'
      if (tag === 'super-admin') return 'warning'
    ...
    const fetchTasks = () => {
      store.dispatch('app-roles/fetchPermissions', {
        q: searchQuery.value,
    ...
    const handlePermissionClick = taskData => {
      task.value = taskData
      isTaskHandlerSidebarActive.value = true
    }

    // Single Task isCompleted update
    const updatePermissionIsCompleted = permissionData => {
      // eslint-disable-next-line no-param-reassign
      permissionData.isCompleted = !permissionData.isCompleted
      updateTask(permissionData)
    }
...
<style lang="scss">
@import "~@core/scss/base/pages/role-permission.scss";
</style>
