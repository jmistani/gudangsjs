<template>
  ...
  >
    ...
    <div class="m-2">
      <b-row class="card-inventory-view-title mb-0 pb-0" style="background: url('/images/dimension.png')');">
        <b-col v-if="!refIsFetching" cols="12" class="card-inventory-view-title mb-0 mt-0 pb-0 pt-0">

            <h3 class="text-primary mb-2">{{refTicketData.name}}</h3>
              <!-- <b-button
                size="sm"
            ...
            <b-card-text v-if="refTicketData.type == 'kendaraan'" class="font-small-3">
              No Polisi: {{refTicketData.nopolisi}}
            </b-card-text>
            <b-media no-body v-if="!refIsFetching">
              ...
              <b-img
                ...
                :src="require('@public/images/binders.svg')"
              />
            </b-media>
          </b-col>
      </b-row>      
    ...
    <div class="m-1">
      ...
      <b-table
        ...
        :items="refTicketData.stock_mutations"
        responsive
        :fields="tableColumns"
        primary-key="id"
        :sort-by.sync="sortBy"
...
export default {
  ...
  methods: {
    recalculate() {
      ...
      store.dispatch('app-inventory/recalculateTicket', { id: router.currentRoute.params.id })
        .then(response => { 
          this.refTicketData = response.data.payload 
  ...
  setup() {
    ...
    } = useTicketView(router.currentRoute.params.id)
