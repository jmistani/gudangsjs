import { VeeValidate, extend, localize } from 'vee-validate'
import {
...
export const qtyConsumeAdd = extend('qtyConsumeAdd', {
    validate(value, { list, prop, key, index }) {
        if (list[index][prop] == null) return null
        let id = list[index][prop][key]
        if (id == null) return null
        if (parseFloat(value) > 0) {
            return axios.get(`/validation/stokBarang/${id}/${value}`).then((response) => {
            ...
            return "Masukkan jumlah dalam angka"
        }
    },
...
export const requiredCode = extend('requiredCode', {
    validate(value) {
        if (value == '' || value == null) {
            ...
            return axios.get(`/validation/inventory-consume/kode/${value}`).then((response) => {
                // Notice that we return an object containing both a valid property and a data property.
                if (response.data.payload.valid == false)
...
export const requiredStaff = extend('requiredStaff', {
    validate(value) {
        if (value == '' || value == null) {
            return "Masukkan nama staff gudang"
        } else {
            return true
        }
    },
    computesRequired: true,
    params: [],
    message: 'Masukkan nama staff gudang'
});
export const requiredReceiver = extend('requiredReceiver', {
    validate(value) {
        if (value == '' || value == null) {
...
export const requiredKeterangan = extend('requiredKeterangan', {
    validate(value) {
        if (value == '' || value == null) {
